define("modules/crm/views/scheduler/scheduler", ["exports", "view", "vis-data", "vis-timeline", "moment", "jquery"], function (_exports, _view, _visData, _visTimeline, _moment, _jquery) {
  ...
  class SchedulerView extends _view.default {
    ...
    getAvatarHtml(id) {
      ...
      return (0, _jquery.default)('<img>').addClass('avatar avatar-link').attr('width', '14').attr('src', this.getBasePath() + '?entryPoint=avatar&size=small&id=' + id + '&t=' + t).get(0).outerHTML;
    }
    getFormatObject() {
      return {
        minorLabels: {
  ...
  var _default = SchedulerView;
  _exports.default = _default;
});

define("modules/crm/views/calendar/timeline", ["exports", "view", "vis-data", "vis-timeline", "moment", "jquery"], function (_exports, _view, _visData, _visTimeline, _moment, _jquery) {
  "use strict";
  ...
  class TimelineView extends _view.default {
    ...
    events = {
      ...
      'click [data-action="mode"]': function (e) {
        let mode = (0, _jquery.default)(e.currentTarget).data('mode');
      ...
      'click [data-action="toggleCalendarType"]': function (e) {
        ...
        let $showSharedCalendarOptions = this.$el.find('> .button-container button[data-action="showSharedCalendarOptions"]');
      ...
      this.fetchEvents(this.start, this.end, eventList => {
        ...
        this.timeline.on('click', e => {
          ...
          if (e.what === 'background' && e.group && e.time) {
            let dateStart = (0, _moment.default)(e.time).utc().format(this.getDateTime().internalDateTimeFormat);
    ...
    createEvent(dateStart, userId) {
      if (!dateStart) {
        let time = (this.timeline.getWindow().end - this.timeline.getWindow().start) / 2 + this.timeline.getWindow().start;
    ...
    viewEvent(scope, id) {
      Espo.Ui.notify(' ... ');
      let viewName = this.getMetadata().get(['clientDefs', scope, 'modalViews', 'detail']) || 'views/modals/detail';
    ...
    actionAddUser() {
      ...
      let viewName = this.getMetadata().get('clientDefs.' + this.foreignScope + '.modalViews.select') || 'views/modals/select-records';
      Espo.Ui.notify(' ... ');
