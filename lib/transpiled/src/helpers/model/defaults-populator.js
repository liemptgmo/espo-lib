define("helpers/model/defaults-populator", ["exports"], function (_exports) {
  "use strict";
      ...
      if (model.hasField('assignedUser') || hasAssignedUsers) {
        ...
        if (this.preferences.get('doNotFillAssignedUserIfNotRequired')) {
          ...
          if (model.getFieldParam(assignedUserField, 'required')) {
            ...
            fillAssignedUser = true;
          } else if (this.acl.getPermissionLevel('assignmentPermission') === 'team' && !this.user.get('defaultTeamId')) {
            fillAssignedUser = true;
          } else if (this.acl.getScopeForbiddenFieldList(model.entityType, 'edit').includes(assignedUserField)) {
            fillAssignedUser = true;
          }
        }
        if (fillAssignedUser) {
  ...
  var _default = DefaultsPopulator;
