define("app", ["exports", "jquery", "backbone", "bullbone", "js-base64", "ui", "utils", "acl-manager", "cache", "storage", "models/settings", "language", "metadata", "field-manager", "models/user", "models/preferences", "model-factory", "collection-factory", "controllers/base", "router", "date-time", "layout-manager", "theme-manager", "session-storage", "view-helper", "web-socket-manager", "ajax", "number-util", "page-title", "broadcast-channel"], function (_exports, _jquery, _backbone, _bullbone, _jsBase, _ui, _utils, _aclManager, _cache, _storage, _settings, _language, _metadata, _fieldManager, _user, _preferences, _modelFactory, _collectionFactory, _base, _router, _dateTime, _layoutManager, _themeManager, _sessionStorage, _viewHelper, _webSocketManager, _ajax, _numberUtil, _pageTitle, _broadcastChannel) {
  "use strict";
  ...
  class App {
    ...
    constructor(options, callback) {
      ...
      this.initCache(options).then(() => this.init(options, callback));
    ...
    initCache(options) {
      ...
      this.cache = new _cache.default(cacheTimestamp);
      const storedCacheTimestamp = this.cache.getCacheTimestamp();
      cacheTimestamp ? this.cache.handleActuality(cacheTimestamp) : this.cache.storeTimestamp();
      if (!window.caches) {
        return Promise.resolve();
      }
      return new Promise(resolve => {
        const deleteCache = !cacheTimestamp || !storedCacheTimestamp || cacheTimestamp !== storedCacheTimestamp;
        (deleteCache ? caches.delete('espo') : Promise.resolve()).then(() => caches.open('espo')).then(cache => {
    ...
    init(options, callback) {
      ...
      Promise.all([this.settings.load(), this.language.loadDefault(), this.initTemplateBundles()]).then(() => {
        ...
        this.themeManager = new _themeManager.default(this.settings, this.preferences, this.metadata);
    ...
    loadStylesheet() {
      ...
      (0, _jquery.default)('#main-stylesheet').attr('href', stylesheetPath);
    }
    ...
    requestUserData(callback) {
      _ajax.default.getRequest('App/user', {}, {
        appStart: true
      }).then(callback);
    }
    ...
    setupAjax() {
      ...
      const onError = (xhr, options) => {
        setTimeout(() => {
          ...
          switch (xhr.status) {
            case 200:
              _ui.default.error(this.language.translate('Bad server response'));
      ...
      _jquery.default.ajaxSetup({
        beforeSend: (xhr, options) => {
          ...
          if (!options.local && this.apiUrl) {
            options.url = _utils.default.trimSlash(this.apiUrl) + '/' + options.url;
          }
    ...
    initDomEventListeners() {
      (0, _jquery.default)(document).on('keydown.espo.button', e => {
