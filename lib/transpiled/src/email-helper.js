define("email-helper", ["exports"], function (_exports) {
  "use strict";
  ...
  class EmailHelper {
    ...
    getReplyAttributes(model, data, cc) {
      ...
      if ((!to || !to.includes('@')) && model.get('from')) {
    ...
    addReplyBodyAttributes(model, attributes) {
      ...
      let replyHeadString = dateSentSting || this.getLanguage().translate('Original message', 'labels', 'Email');
