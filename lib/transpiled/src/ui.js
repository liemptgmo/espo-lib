define("ui", ["exports", "marked", "dompurify", "jquery"], function (_exports, _marked, _dompurify, _jquery) {
  ...
  Espo.Ui = {
      ...
      $el.popover({
        ...
        if (view && !o.noHideOnOutsideClick) {
          ...
          $body.on('click.popover-' + view.cid, e => {
            if ((0, _jquery.default)(e.target).closest('.popover-content').get(0)) {
              return;
            }
    ...
    notify: function (message, type, timeout, options) {
      ...
      if (closeButton) {
        const $close = (0, _jquery.default)('<button>').attr('type', 'button').attr('data-dismiss', 'modal').attr('aria-hidden', 'true').addClass('close').html('&times;');
