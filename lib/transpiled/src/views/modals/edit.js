define("views/modals/edit", ["exports", "views/modal", "backbone"], function (_exports, _modal, _backbone) {
  ...
  class EditModalView extends _modal.default {
    ...
    createRecordView(model, callback) {
      ...
      this.createView('edit', viewName, options, callback).then(view => {
    ...
    composeHeaderHtml() {
      ...
      if (!this.id) {
        ...
        const text = this.getLanguage().translate('Edit') + ' · ' + this.getLanguage().translate(this.scope, 'scopeNames');
        html = $('<span>').text(text).get(0).outerHTML;
      }
      if (!this.fullFormDisabled) {
    ...
    actionSave(data) {
      ...
      editView.save().then(() => {
        ...
        this.$el.find('.modal-header .modal-title-text').html(this.composeHeaderHtml());
