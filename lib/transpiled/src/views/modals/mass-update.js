define("views/modals/mass-update", ["exports", "views/modal", "helpers/mass-action", "ui/select"], function (_exports, _modal, _massAction, _select) {
  ...
  class MassUpdateModalView extends _modal.default {
    ...
    setup() {
      ...
      this.$header = $('<span>').append($('<span>').text(this.translate(this.scope, 'scopeNamesPlural')), ' <span class="chevron-right"></span> ', $('<span>').text(this.translate('Mass Update')));
    ...
    actionUpdate() {
      ...
      Espo.Ajax.postRequest('MassAction', {
        ...
        result = result || {};
        if (result.id) {
          this.helper.process(result.id, 'update').then(view => {
  ...
  var _default = MassUpdateModalView;
