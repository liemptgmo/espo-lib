define("views/modals/detail", ["exports", "views/modal", "helpers/action-item-setup", "backbone"], function (_exports, _modal, _actionItemSetup, _backbone) {
  "use strict";
  ...
  class DetailModalView extends _modal.default {
    ...
    createRecordView(callback) {
      ...
      if (model.get('name')) {
        ...
        this.headerHtml += ' ' + $('<span>').text(model.get('name')).get(0).outerHTML;
      }
      if (!this.fullFormDisabled) {
      ...
      const viewName = this.detailViewName || this.detailView || this.getMetadata().get(['clientDefs', model.entityType, 'recordViews', 'detailSmall']) || this.getMetadata().get(['clientDefs', model.entityType, 'recordViews', 'detailQuick']) || 'views/record/detail-small';
      const options = {
        model: model,
