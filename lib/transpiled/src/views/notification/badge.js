define("views/notification/badge", ["exports", "view"], function (_exports, _view) {
  ...
  class NotificationBadgeView extends _view.default {
    ...
    checkBypass() {
      ...
      if (last.controller === 'Admin' && last.action === 'page' && ~['upgrade', 'extensions'].indexOf(pageAction)) {
        return true;
      }
      return false;
    }
    checkUpdates(isFirstCheck) {
      ...
      Espo.Ajax.getRequest('Notification/action/notReadCount').then(count => {
        if (!isFirstCheck && count > this.unreadCount) {
          const messageBlockPlayNotificationSound = localStorage.getItem('messageBlockPlayNotificationSound');
  ...
  var _default = NotificationBadgeView;
