define("views/record/panels/default-side", ["exports", "views/record/panels/side"], function (_exports, _side) {
  ...
  class DefaultSidePanelView extends _side.default {
    ...
    setup() {
      ...
      this.hasComplexCreated = !!this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', 'createdAt']) && !!this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', 'createdBy']);
      this.hasComplexModified = !!this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', 'modifiedAt']) && !!this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', 'modifiedBy']);
      super.setup();
    }
    setupFields() {
      super.setupFields();
  ...
  var _default = DefaultSidePanelView;
