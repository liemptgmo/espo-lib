define("views/record/panels/relationship", ["exports", "views/record/panels/bottom", "search-manager", "helpers/record-modal"], function (_exports, _bottom, _searchManager, _recordModal) {
  "use strict";
  ...
  class RelationshipPanelView extends _bottom.default {
    ...
    setup() {
      ...
      const linkReadOnly = this.getMetadata().get(['entityDefs', this.model.entityType, 'links', this.link, 'readOnly']) || false;
      ...
      this.getCollectionFactory().create(this.entityType, collection => {
        ...
        if (this.defs.filters) {
          const searchManager = new _searchManager.default(collection, 'listRelationship', null, this.getDateTime());
    ...
    actionViewRelatedList(data) {
      const viewName = this.getMetadata().get(['clientDefs', this.model.entityType, 'relationshipPanels', this.name, 'viewModalView']) || this.getMetadata().get(['clientDefs', this.entityType, 'modalViews', 'relatedList']) || this.viewModalView || 'views/modals/related-list';
  ...
  var _default = RelationshipPanelView;
