define("views/record/list", ["exports", "view", "helpers/mass-action", "helpers/export", "helpers/record-modal", "helpers/list/select-provider", "views/record/list/settings", "helpers/list/settings"], function (_exports, _view, _massAction, _export, _recordModal, _selectProvider, _settings, _settings2) {
  "use strict";
  ...
  class ListRecordView extends _view.default {
    ...
    /** @inheritDoc */
    data() {
      const paginationTop = this.pagination === 'both' || this.pagination === 'top';
      const paginationBottom = this.pagination === 'both' || this.pagination === true || this.pagination === 'bottom';
      ...
      const topBar = paginationTop || this.checkboxes || this.buttonList.length && !this.buttonsDisabled || this.dropdownItemList.length && !this.buttonsDisabled || this.forceDisplayTopBar || displayTotalCount;
    ...
    init() {
      ...
      this.pagination = _.isUndefined(this.options.pagination) || this.options.pagination === null ? this.pagination : this.options.pagination;
    ...
    setupMassActions() {
      ...
      metadataMassActionList.forEach(item => {
        ...
        if (!Espo.Utils.checkActionAvailability(this.getHelper(), defs) || !Espo.Utils.checkActionAccess(this.getAcl(), null, defs)) {
          return;
        }
    ...
    getItemEl(model, item) {
      return this.getSelector() + ' tr[data-id="' + model.id + '"]' + ' td.cell[data-name="' + item.columnName + '"]';
    }
    ...
    showMoreRecords(options, collection, $list, $showMore, callback) {
      ...
      const final = () => {
        ...
        if (collection.total > collection.length + collection.lengthCorrection || collection.total === -1) {
    ...
    actionQuickView(data) {
      ...
      const helper = new _recordModal.default(this.getMetadata(), this.getAcl());
    ...
    setupSettings() {
      ...
      if (!this.getMetadata().get(`scopes.${this.entityType}.object`) || this.getConfig().get('listViewSettingsDisabled')) {
        return;
      }
      this._listSettingsHelper = new _settings2.default(this.entityType, this.layoutName, this.getUser().id, this.getStorage());
