define("views/record/detail", ["exports", "views/record/base", "view-record-helper", "helpers/action-item-setup"], function (_exports, _base, _viewRecordHelper, _actionItemSetup) {
  "use strict";
  ...
  class DetailRecordView extends _base.default {
    ...
    setupActionItems() {
      ...
      if (this.type === this.TYPE_DETAIL) {
        const actionItemSetup = new _actionItemSetup.default(this.getMetadata(), this.getHelper(), this.getAcl(), this.getLanguage());
    ...
    showPanel(name, softLockedType) {
      ...
      this.recordHelper.setPanelStateParam(name, 'hidden' + Espo.Utils.upperCaseFirst(softLockedType) + 'Locked', false);
    ...
    hidePanel(name, locked, softLockedType) {
      ...
      if (softLockedType) {
        this.recordHelper.setPanelStateParam(name, 'hidden' + Espo.Utils.upperCaseFirst(softLockedType) + 'Locked', true);
      }
      if (softLockedType === 'dynamicLogic') {
    ...
    initStickableButtonsContainer() {
      ...
      const $block = $('<div>').css('height', blockHeight + 'px').html('&nbsp;').hide().insertAfter($container);
    ...
    setup() {
      ...
      this.recordHelper = this.options.recordHelper || new _viewRecordHelper.default(this.defaultFieldStates, this.defaultFieldStates);
    ...
    addDropdownItem(o, toBeginning) {
      if (!o) {
        toBeginning ? this.dropdownItemList.unshift(false) : this.dropdownItemList.push(false);
        return;
      }
      const name = o.name;
      if (!name) {
