define("views/record/kanban", ["exports", "views/record/list"], function (_exports, _list) {
  "use strict";
  ...
  class KanbanRecordView extends _list.default {
    ...
    afterRender() {
      ...
      this.statusList.forEach(status => {
        ...
        this.plusElementMap[status] = this.$el.find('.kanban-head .create-button[data-group="' + value + '"]');
      });
    }
    initStickableHeader() {
    ...
    actionCreateInGroup(group) {
      ...
      this.createView('quickCreate', viewName, options, /** module:views/modals/edit */view => {
        ...
        this.listenToOnce(view, 'after:save', () => {
          ...
          this.storeGroupOrder(group, view.model.id).then(() => this.collection.fetch());
        });
      });
    }
    initBackDrag(e) {
