define("views/record/base", ["exports", "view", "view-record-helper", "dynamic-logic", "underscore", "jquery", "helpers/model/defaults-populator"], function (_exports, _view, _viewRecordHelper, _dynamicLogic, _underscore, _jquery, _defaultsPopulator) {
  ...
  class BaseRecordView extends _view.default {
    ...
    save(options) {
      ...
      const attributes = _underscore.default.extend(Espo.Utils.cloneDeep(beforeSaveAttributes), fetchedAttributes);
      let setAttributes = {};
      if (model.isNew()) {
      ...
      if (!model.isNew()) {
        for (const attr in attributes) {
          if (_underscore.default.isEqual(initialAttributes[attr], attributes[attr])) {
            continue;
          }
     ...
     */
    populateDefaults() {
      const populator = new _defaultsPopulator.default(this.getUser(), this.getPreferences(), this.getAcl(), this.getConfig());
