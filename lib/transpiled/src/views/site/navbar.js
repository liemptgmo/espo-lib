define("views/site/navbar", ["exports", "view", "jquery"], function (_exports, _view, _jquery) {
  "use strict";
  ...
  class NavbarSiteView extends _view.default {
    ...
    openSideMenu() {
      ...
      this.$sideMenuBackdrop = (0, _jquery.default)('<div>').addClass('side-menu-backdrop').click(() => this.closeSideMenu()).appendTo(this.$body);
    ...
    getTabList() {
      let tabList = this.getPreferences().get('useCustomTabList') ? this.getPreferences().get('tabList') : this.getConfig().get('tabList');
    ...
    adjustSide() {
      ...
      (0, _jquery.default)(window).on('resize.navbar', () => {
        updateSizeForSide();
      });
      updateSizeForSide();
      this.adjustBodyMinHeight();
    ...
    setupTabDefsList() {
      ...
      const tabDefsList = [];
      const colorsDisabled = this.getPreferences().get('scopeColorsDisabled') || this.getPreferences().get('tabColorsDisabled') || this.getConfig().get('scopeColorsDisabled') || this.getConfig().get('tabColorsDisabled');
