define("views/list-with-categories", ["exports", "views/list"], function (_exports, _list) {
  ...
  class ListWithCategories extends _list.default {
    ...
    openCategory(id, name) {
      ...
      if (this.nestedCategoriesCollection) {
        ...
        Espo.Ui.notify(' ... ');
        Promise.all([this.nestedCategoriesCollection.fetch().then(() => this.updateHeader()), this.collection.fetch({
          openCategory: true
        })]).then(() => {
    ...
    getHeader() {
      ...
      const $root = $('<a>').attr('href', rootUrl).addClass('action').text(this.translate(this.scope, 'scopeNamesPlural')).addClass('action').attr('data-action', 'openCategory');
      const list = [$root];
      ...
      if (upperId) {
        ...
        const $folder = $('<a>').attr('href', url).text(upperName).addClass('action').attr('data-action', 'openCategory').attr('data-id', upperId).attr('data-name', upperName);
        list.push($folder);
      }
  ...
  var _default = ListWithCategories;
  _exports.default = _default;
});
//# sourceMappingURL=list-with-categories.js.map ;