define("views/stream/note", ["exports", "view"], function (_exports, _view) {
  ...
  class NoteStreamView extends _view.default {
    ...
    getIconHtml(scope, id) {
      ...
      return $('<span>').addClass(iconClass).addClass('action text-muted icon').css('cursor', 'pointer').attr('title', this.translate('View')).attr('data-action', 'quickView').attr('data-id', id).attr('data-scope', scope).get(0).outerHTML;
    }
  }
  var _default = NoteStreamView;
