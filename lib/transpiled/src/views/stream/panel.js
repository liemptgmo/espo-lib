define("views/stream/panel", ["exports", "views/record/panels/relationship", "jquery-textcomplete"], function (_exports, _relationship, _jqueryTextcomplete) {
  "use strict";
  ...
  _jqueryTextcomplete = _interopRequireDefault(_jqueryTextcomplete);
  ...
  class PanelStreamView extends _relationship.default {
    ...
    afterRender() {
      ...
      let buildUserListUrl = term => {
        var url = 'User?orderBy=name&limit=7&q=' + term + '&' + $.param({
      ...
      let syntaxItemList = [['code', '`{text}`'], ['multilineCode', '```{text}```'], ['strongText', '**{text}**'], ['emphasizedText', '*{text}*'], ['deletedText', '~~{text}~~'], ['blockquote', '> {text}'], ['link', '[{text}](url)']];
      let messageItemList = [];
      ...
      let $ul = $('<ul>').append(messageItemList.map(text => $('<li>').text(text)));
      let messageHtml = this.getHelper().transformMarkdownInlineText(text1) + '<br><br>' + this.getHelper().transformMarkdownInlineText(text2) + ':<br>' + $ul.get(0).outerHTML;
      Espo.Ui.popover($a, {
        content: messageHtml
      }, this);
    ...
    setupActions() {
      ...
      this.actionList.push({
        action: 'viewPostList',
        html: $('<span>').append($('<span>').text(this.translate('View List')), ' &middot; ', $('<span>').text(this.translate('posts', 'filters', 'Note'))).get(0).innerHTML,
