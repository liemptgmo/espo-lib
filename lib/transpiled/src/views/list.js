define("views/list", ["exports", "views/main", "search-manager"], function (_exports, _main, _searchManager) {
  "use strict";
  ...
  class ListView extends _main.default {
    ...
    setupSearchManager() {
      ...
      const searchManager = new _searchManager.default(collection, 'list', this.getStorage(), this.getDateTime(), this.getSearchDefaultData());
     ...
     */
    getHeader() {
      const $root = $('<span>').text(this.getLanguage().translate(this.scope, 'scopeNamesPlural'));
      if (this.options.params && this.options.params.fromAdmin) {
        const $root = $('<a>').attr('href', '#Admin').text(this.translate('Administration', 'labels', 'Admin'));
        const $scope = $('<span>').text(this.getLanguage().translate(this.scope, 'scopeNamesPlural'));
