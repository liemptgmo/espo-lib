define("views/edit", ["exports", "views/main"], function (_exports, _main) {
  ...
  class EditView extends _main.default {
    ...
    /** @inheritDoc */
    optionsToPass = ['returnUrl', 'returnDispatchParams', 'attributes', 'rootUrl', 'duplicateSourceId', 'returnAfterCreate'];
    ...
    getHeader() {
      ...
      if (!this.options.noHeaderLinks && !this.rootLinkDisabled) {
        $root = $('<span>').append($('<a>').attr('href', rootUrl).addClass('action').attr('data-action', 'navigateToRoot').text(scopeLabel));
      }
      if (headerIconHtml) {
      ...
      if (!this.options.noHeaderLinks) {
        ...
        $name = $('<a>').attr('href', url).addClass('action').append($name);
      }
