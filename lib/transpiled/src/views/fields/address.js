define("views/fields/address", ["exports", "views/fields/base", "views/fields/varchar"], function (_exports, _base, _varchar) {
  "use strict";
  ...
  class AddressFieldView extends _base.default {
      ...
      let validate = name => {
        if (this.model.isRequired(name)) {
          if (this.model.get(name) === '') {
            let msg = this.translate('fieldIsRequired', 'messages').replace('{field}', this.translate(name, 'fields', this.entityType));
    ...
    validatePattern() {
      let fieldList = [this.postalCodeField, this.stateField, this.cityField, this.countryField];
      let result = false;
    ...
    fetch() {
      ...
      let attributeList = [this.postalCodeField, this.streetField, this.stateField, this.cityField, this.countryField];
    ...
    fetchSearch() {
      let value = this.$el.find('input.main-element').val().toString().trim();
      if (!value) {
        return null;
      }
      return {
