define("views/fields/link-multiple-with-role", ["exports", "views/fields/link-multiple", "ui/select"], function (_exports, _linkMultiple, _select) {
  ...
  class LinkMultipleWithRoleFieldView extends _linkMultiple.default {
    ...
    getDetailLinkHtml(id, name) {
      ...
      if (role) {
        let style = this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', this.roleField, 'style', role]);
        let className = 'text';
        ...
        let text = this.roleType === this.ROLE_TYPE_ENUM ? this.getLanguage().translateOption(role, this.roleField, this.roleFieldScope) : role;
        $el.append($('<span>').text(' '), $('<span>').addClass('text-muted chevron-right'), $('<span>').text(' '), $('<span>').text(text).addClass('small').addClass(className));
      }
    ...
    getJQSelect(id, roleValue) {
      ...
      let $role = $('<select>').addClass('role form-control input-sm').attr('data-id', id);
      this.roleList.forEach(role => {
        let text = this.getLanguage().translateOption(role, this.roleField, this.roleFieldScope);
        let $option = $('<option>').val(role).text(text);
        if (role === (roleValue || '')) {
  ...
  var _default = LinkMultipleWithRoleFieldView;
  _exports.default = _default;
});
//# sourceMappingURL=link-multiple-with-role.js.map ;