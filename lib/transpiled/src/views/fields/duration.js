define("views/fields/duration", ["exports", "views/fields/enum", "ui/select", "moment"], function (_exports, _enum, _select, _moment) {
  ...
  class DurationFieldView extends _enum.default {
    ...
    init() {
      ...
      this.listenTo(this, 'render', () => {
        ...
        this.getOptions().forEach(d => {
          let $o = $('<option>').val(d).text(this.stringifyDuration(d));
          if (d === this.seconds) {
    ...
    setup() {
      ...
      this.listenTo(this.model, 'change:' + this.endField, (m, v, o) => {
        ...
        this.seconds = (0, _moment.default)(end).unix() - (0, _moment.default)(start).unix();
    ...
    _getDateEndDate() {
      ...
      let endUnix = _moment.default.utc(start).unix() + seconds;
    ...
    updateDateEnd() {
      ...
      if (this.model.get('isAllDay')) {
        ...
        setTimeout(() => {
          this.model.set(this.endField + 'Date', end, {
  ...
  var _default = DurationFieldView;
