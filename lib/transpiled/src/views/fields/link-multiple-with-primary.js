define("views/fields/link-multiple-with-primary", ["exports", "views/fields/link-multiple"], function (_exports, _linkMultiple) {
  ...
  class LinkMultipleWithPrimaryFieldView extends _linkMultiple.default {
    ...
    getValueForDisplay() {
      if (this.isDetailMode() || this.isListMode()) {
        ...
        return itemList.map(item => $('<div>').append(item).get(0).outerHTML).join('');
      }
    }
    ...
    addLinkHtml(id, name) {
      ...
      let $el = $('<div>').addClass('form-inline clearfix ').addClass('list-group-item link-with-role link-group-item-with-primary').addClass('link-' + id).attr('data-id', id);
      ...
      let $star = $('<span>').addClass('fas fa-star fa-sm').addClass(!isPrimary ? 'text-muted' : '');
      let $button = $('<button>').attr('type', 'button').addClass('btn btn-link btn-sm pull-right hidden').attr('title', this.translate('Primary')).attr('data-action', 'switchPrimary').attr('data-id', id).append($star);
  ...
  var _default = LinkMultipleWithPrimaryFieldView;
  _exports.default = _default;
});
//# sourceMappingURL=link-multiple-with-primary.js.map ;