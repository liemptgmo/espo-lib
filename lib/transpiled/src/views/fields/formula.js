  ...
  class FormulaFieldView extends _text.default {
    ...
    requireAce() {
      return Espo.loader.requirePromise('lib!ace').then(lib => {
        ace = /** window.ace */lib;
        let list = [Espo.loader.requirePromise('lib!ace-mode-javascript'), Espo.loader.requirePromise('lib!ace-ext-language_tools')];
    ...
    getFunctionDataList() {
      let list = Espo.Utils.clone(this.getMetadata().get(['app', 'formula', 'functionList']) || []);
    ...
    initAutocomplete() {
      ...
      let completer = {
        ...
        getCompletions: function (editor, session, pos, prefix, callback) {
          let matchedFunctionItemList = functionItemList.filter(originalItem => {
    ...
    getFormulaAttributeList() {
      ...
      let attributeList = this.getFieldManager().getEntityTypeAttributeList(this.targetEntityType).concat(['id']).sort();
      ...
      linkList.forEach(link => {
        ...
        let linkAttributeList = this.getFieldManager().getEntityTypeAttributeList(scope).sort();
