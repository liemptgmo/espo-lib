define("views/fields/complex-expression", ["exports", "views/fields/text"], function (_exports, _text) {
  ...
  class ComplexExpressionFieldView extends _text.default {
    ...
    requireAce() {
      return Espo.loader.requirePromise('lib!ace').then(lib => {
        ace = lib;
        let list = [Espo.loader.requirePromise('lib!ace-ext-language_tools')];
    ...
    initAutocomplete() {
      let functionItemList = this.getFunctionDataList().filter(item => {
    ...
    getFormulaAttributeList() {
      ...
      let attributeList = this.getFieldManager().getEntityTypeAttributeList(this.targetEntityType).sort();
  ...
  var _default = ComplexExpressionFieldView;
  _exports.default = _default;
});
//# sourceMappingURL=complex-expression.js.map ;