define("views/fields/link-multiple-with-columns", ["exports", "views/fields/link-multiple", "helpers/reg-exp-pattern", "ui/select"], function (_exports, _linkMultiple, _regExpPattern, _select) {
  "use strict";
  ...
  class LinkMultipleWithColumnsFieldView extends _linkMultiple.default {
    ...
    setup() {
      ...
      let columns = this.getMetadata().get(['entityDefs', this.model.entityType, 'fields', this.name, 'columns']) || {};
    ...
    getDetailLinkHtml(id, name) {
      ...
      let $el = $('<div>').append($('<a>').attr('href', '#' + this.foreignScope + '/view/' + id).attr('data-id', id).text(name));
      ...
      this.columnList.forEach(column => {
        ...
        let text = type === this.COLUMN_TYPE_ENUM ? this.getLanguage().translateOption(value, field, scope) : value;
        $el.append($('<span>').text(' '), $('<span>').addClass('text-muted chevron-right'), $('<span>').text(' '), $('<span>').text(text).addClass('text-muted small'));
      });
    ...
    getJQSelect(column, id, value) {
      ...
      let $select = $('<select>').addClass('role form-control input-sm').attr('data-id', id).attr('data-column', column);
      options.forEach(itemValue => {
        let text = this.getLanguage().translateOption(itemValue, field, scope);
        let $option = $('<option>').val(itemValue).text(text);
        if (itemValue === (value || '')) {
    ...
    getJQInput(column, id, value) {
      ...
      let $input = $('<input>').addClass('role form-control input-sm').attr('data-column', column).attr('placeholder', text).attr('data-id', id).attr('value', value || '');
      if (maxLength) {
    ...
    addLinkHtml(id, name) {
      ...
      let $el = $('<div>').addClass('form-inline clearfix').addClass('list-group-item link-with-role link-group-item-with-columns').addClass('link-' + id);
      let $remove = $('<a>').attr('role', 'button').attr('tabindex', '0').attr('data-id', id).attr('data-action', 'clearLink').addClass('pull-right').append($('<span>').addClass('fas fa-times'));
      let $name = $('<div>').addClass('link-item-name').text(name).append('&nbsp;');
      let $columnList = [];
      let $liList = [];
      this.columnList.forEach(column => {
        let value = (this.columns[id] || {})[column];
    ...
    validateColumnPattern() {
      let result = false;
      let columnList = this.columnList.filter(column => this.columnsDefs[column].type === this.COLUMN_TYPE_VARCHAR).filter(column => this.columnsDefs[column].pattern);
    ...
    validateColumnPatternValue(id, column, value) {
      ...
      let field = this.columnsDefs[column].field;
      let scope = this.columnsDefs[column].scope;
      let helper = new _regExpPattern.default(this.getMetadata(), this.getLanguage());
  ...
  var _default = LinkMultipleWithColumnsFieldView;
  _exports.default = _default;
});
//# sourceMappingURL=link-multiple-with-columns.js.map ;