define("views/fields/date", ["exports", "views/fields/base", "moment"], function (_exports, _base, _moment) {
  "use strict";
  ...
  class DateFieldView extends _base.default {
    ...
    convertDateValueForDetail(value) {
      ...
      let today = (0, _moment.default)().tz(timezone).startOf('day');
    ...
    afterRender() {
      if (this.mode === this.MODE_EDIT || this.mode === this.MODE_SEARCH) {
        ...
        let options = {
          ...
          container: this.$el.closest('.modal-body').length ? this.$el.closest('.modal-body') : 'body'
        };
    ...
    validateDate() {
      if (this.model.get(this.name) === -1) {
        let msg = this.translate('fieldShouldBeDate', 'messages').replace('{field}', this.getLabelText());
        this.showValidationMessage(msg);
        return true;
      }
    }
    ...
    validateAfter() {
      ...
      if ((0, _moment.default)(value).unix() <= (0, _moment.default)(otherValue).unix()) {
        let msg = this.translate('fieldShouldAfter', 'messages').replace('{field}', this.getLabelText()).replace('{otherField}', this.translate(field, 'fields', this.entityType));
        this.showValidationMessage(msg);
        return true;
      }
    }
    ...
    validateBefore() {
      ...
      if ((0, _moment.default)(value).unix() >= (0, _moment.default)(otherValue).unix()) {
        let msg = this.translate('fieldShouldBefore', 'messages').replace('{field}', this.getLabelText()).replace('{otherField}', this.translate(field, 'fields', this.entityType));
        this.showValidationMessage(msg);
        return true;
      }
    }
  }
  var _default = DateFieldView;
