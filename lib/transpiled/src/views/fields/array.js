  ...
  class ArrayFieldView extends _base.default {
    ...
    addValueFromUi(value) {
      ...
      if (this.params.pattern) {
        const helper = new _regExpPattern.default(this.getMetadata(), this.getLanguage());
    ...
    getValueForDisplay() {
      ...
      const list = this.selected.map(item => {
        ...
        if (this.params.displayAsLabel) {
          return $('<span>').addClass('label label-md label-' + style).text(label).get(0).outerHTML;
        }
    ...
    getItemHtml(value) {
      ...
      return $('<div>').addClass('list-group-item').attr('data-value', value).css('cursor', 'default').append($('<a>').attr('role', 'button').attr('tabindex', '0').addClass('pull-right').attr('data-value', value).attr('data-action', 'removeValue').append($('<span>').addClass('fas fa-times'))).append($('<span>').addClass('text').text(text)).append('').get(0).outerHTML;
    }
    addValue(value) {
