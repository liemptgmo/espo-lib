define("views/fields/base", ["exports", "view", "ui/select", "jquery"], function (_exports, _view, _select, _jquery) {
  ...
  class BaseFieldView extends _view.default {
    ...
    initTooltip() {
      ...
      this.once('after:render', () => {
        ...
        if (!tooltipText && typeof this.tooltip === 'string') {
          let [scope, field] = this.tooltip.includes('.') ? this.tooltip.split('.') : [this.entityType, this.tooltip];
    ...
    initInlineEdit() {
      let $cell = this.get$cell();
      let $editLink = (0, _jquery.default)('<a>').attr('role', 'button').addClass('pull-right inline-edit-link hidden').append((0, _jquery.default)('<span>').addClass('fas fa-pencil-alt fa-sm'));
      if ($cell.length === 0) {
        this.listenToOnce(this, 'after:render', () => this.initInlineEdit());
        return;
      }
    ...
    addInlineEditLinks() {
      let $cell = this.get$cell();
      let $saveLink = (0, _jquery.default)('<a>').attr('role', 'button').attr('tabindex', '-1').addClass('pull-right inline-save-link').attr('title', 'Ctrl+Enter').text(this.translate('Update'));
      let $cancelLink = (0, _jquery.default)('<a>').attr('role', 'button').attr('tabindex', '-1').addClass('pull-right inline-cancel-link').attr('title', 'Esc').text(this.translate('Cancel'));
    ...
    inlineEditClose(noReset) {
      ...
      let promise = this.setDetailMode().then(() => this.reRender(true)).then(() => this.removeInlineEditLinks());
    ...
    inlineEdit() {
      ...
      let promise = this.setEditMode().then(() => this.reRender(true)).then(() => this.addInlineEditLinks()).then(() => {
     ...
     */
    focusOnInlineEdit() {
      let $element = this.$element && this.$element.length ? this.$element : this.$el.find('.form-control').first();
      if (!$element) {
        return;
      }
      $element.first().focus();
