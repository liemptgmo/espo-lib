define("views/fields/file", ["exports", "views/fields/link", "helpers/file-upload"], function (_exports, _link, _fileUpload) {
  "use strict";
  ...
  class FileFieldView extends _link.default {
    ...
    getDetailPreview(name, type, id) {
      ...
      let $img = $('<img>').attr('src', src).addClass('image-preview').css({
    ...
    getValueForDisplay() {
      ...
      if (this.showPreview && ~this.previewTypeList.indexOf(type)) {
        ...
        let $item = $('<div>').addClass('attachment-preview').addClass(className).append(this.getDetailPreview(name, type, id));
