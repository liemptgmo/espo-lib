define("views/fields/attachment-multiple", ["exports", "views/fields/base", "helpers/file-upload"], function (_exports, _base, _fileUpload) {
  "use strict";
  ...
  class AttachmentMultipleFieldView extends _base.default {
    ...
    getEditPreview(name, type, id) {
      ...
      return $('<img>').attr('src', this.getImageUrl(id, 'small')).attr('title', name).attr('draggable', false).css({
    ...
    addAttachmentBox(name, type, id) {
      ...
      let $remove = $('<a>').attr('role', 'button').attr('tabindex', '0').addClass('remove-attachment pull-right').append($('<span>').addClass('fas fa-times'));
      let previewHtml = this.getBoxPreviewHtml(name, type, id);
      let $att = $('<div>').addClass('gray-box').append($remove).append($('<span>').addClass('preview').append(previewHtml));
      ...
      let $loading = $('<span>').addClass('small uploading-message').text(this.translate('Uploading...'));
    ...
    uploadFiles(files) {
      ...
      if (exceedsMaxFileSize) {
        let msg = this.translate('fieldMaxFileSizeError', 'messages').replace('{field}', this.getLabelText()).replace('{max}', maxFileSize);
      ...
      this.getModelFactory().create('Attachment', model => {
        ...
        let uploadHelper = new _fileUpload.default(this.getConfig());
    ...
    getDetailPreview(name, type, id) {
      ...
      return $('<a>').attr('data-action', 'showImagePreview').attr('data-id', id).attr('title', name).attr('href', this.getImageUrl(id)).append($('<img>').attr('src', this.getImageUrl(id, this.previewSize)).addClass('image-preview').css({
        maxWidth: (this.imageSizes[this.previewSize] || {})[0],
        maxHeight: (this.imageSizes[this.previewSize] || {})[1]
      })).get(0).outerHTML;
    }
    getValueForDisplay() {
      if (this.isDetailMode() || this.isListMode()) {
        ...
        for (let id in nameHash) {
          ...
          if (this.showPreviews && ~this.previewTypeList.indexOf(type) && (this.isDetailMode() || this.isListMode() && this.showPreviewsInListMode)) {
        ...
        let $container = $('<div>').append($('<div>').addClass('attachment-block-container').addClass(containerClassName).append(previews)).append(names);
        return $container.get(0).innerHTML;
      }
    }
    insertFromSource(source) {
      let viewName = this.getMetadata().get(['clientDefs', 'Attachment', 'sourceDefs', source, 'insertModalView']) || this.getMetadata().get(['clientDefs', source, 'modalViews', 'select']) || 'views/modals/select-records';
      if (viewName) {
        Espo.Ui.notify(' ... ');
        let filters = null;
        ...
        let boolFilterList = this.getMetadata().get(['clientDefs', 'Attachment', 'sourceDefs', source, 'boolFilterList']);
        if ('getSelectBoolFilterList' + source in this) {
          boolFilterList = this['getSelectBoolFilterList' + source]();
        }
        let primaryFilterName = this.getMetadata().get(['clientDefs', 'Attachment', 'sourceDefs', source, 'primaryFilter']);
        ...
        this.createView('insertFromSource', viewName, {
          ...
          this.listenToOnce(view, 'select', modelList => {
            ...
            modelList.forEach(model => {
              ...
              Espo.Ajax.postRequest(source + '/action/getAttachmentList', {
                id: model.id,
                field: this.name,
    ...
    validateReady() {
      if (this.isUploading) {
        let msg = this.translate('fieldIsUploading', 'messages').replace('{field}', this.getLabelText());
        this.showValidationMessage(msg, 'label');
        return true;
      }
    }
  ...
  var _default = AttachmentMultipleFieldView;
  _exports.default = _default;
});
//# sourceMappingURL=attachment-multiple.js.map ;