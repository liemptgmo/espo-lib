define("controller", ["exports", "exceptions", "bullbone", "jquery"], function (_exports, _exceptions, _bullbone, _jquery) {
  ...
  class Controller {
    ...
    _unchainMainView(masterView) {
      ...
      if (!masterView.currentViewKey || !this.hasStoredMainView(masterView.currentViewKey)) {
        return;
      }
    ...
    _processMain(mainView, masterView, dto) {
      ...
      if (masterView.currentViewKey) {
        this.set('storedScrollTop-' + masterView.currentViewKey, (0, _jquery.default)(window).scrollTop());
        if (!dto.isSet) {
          this._unchainMainView(masterView);
        }
      }
      ...
      const afterRender = () => {
        ...
        if (dto.useStored && this.has('storedScrollTop-' + key)) {
          (0, _jquery.default)(window).scrollTop(this.get('storedScrollTop-' + key));
          return;
        }
