define("view-helper", ["exports", "marked", "dompurify", "handlebars"], function (_exports, _marked, _dompurify, _handlebars) {
  ...
  class ViewHelper {
    ...
    _registerHandlebarsHelpers() {
      ...
      _handlebars.default.registerHelper('ifNotEmptyHtml', function (value, options) {
      ...
      _handlebars.default.registerHelper('options', (list, value, options) => {
    ...
    getAvatarHtml(id, size, width, additionalClassName) {
      ...
      return $(`<img>`).attr('src', `${basePath}?entryPoint=avatar&size=${size}&id=${id}&t=${t}`).attr('alt', 'avatar').addClass(className).attr('width', width.toString()).get(0).outerHTML;
    }
    ...
    transformMarkdownText(text, options) {
      text = text || '';
      text = _handlebars.default.Utils.escapeExpression(text).replace(/&gt;+/g, '>');
      this.mdBeforeList.forEach(item => {
        text = text.replace(item.regex, item.value);
      });
      options = options || {};
      if (options.inline) {
        text = _marked.marked.parseInline(text);
      } else {
        text = _marked.marked.parse(text);
      }
